...
### Hierarchical intelligence backend

`src/intelligence/hierarchical.py` provides the replay training seam for the
four-level decision framework. It consumes only public rival telemetry
(speed, throttle, brake, gap, and an active-aero/DRS proxy), extracts rolling
features, and runs a 40-state forward HMM over
...
- **Focus / Battle mode**: **F** to toggle highlight of selected driver + 2 cars ahead/behind with gap connectors
...
1. Click one driver in the leaderboard. That driver becomes the focus car; the car immediately ahead becomes the inferred rival.
