...
from src.intelligence.config import load_config
...
class F1RaceReplayWindow(arcade.Window):
    def __init__(self, frames, track_statuses, example_lap, drivers, title,
        ...
        # Dev/test hook: preselect the opening leader so the Race Engineer HUD is
        # visible immediately (only when F1_AUTOFOCUS is set).
        if os.environ.get("F1_AUTOFOCUS") and frames and frames[0].get("drivers"):
            _d0 = frames[0]["drivers"]
            _leader = max(_d0, key=lambda c: _d0[c].get("dist", 0.0))
            self.selected_drivers = [_leader]
            self.selected_driver = _leader
        ...
        # Focus / Battle mode: highlight selected driver + N cars ahead/behind
        # so you can watch overtakes / defence around your team's car.
        # F toggles it. Auto-activates when a driver is selected.
        self.focus_mode = True
        self.focus_radius = 2
        self._battle_codes_set = set()
        self._battle_ordered = []
        self._battle_screen_pos = {}
        # AI Motorsport Intelligence: estimated energy + overtake decision engine
        # (uses a calibrated config if computed_data/model_config.json exists)
        self._model_config = load_config()
        ...
        arcade.set_background_color(CARBON)
    ...
    def _broadcast_telemetry_state(self):
        ...
        payload = {
            ...
            "selected_drivers": list(
                ...
                or ([self.selected_driver] if getattr(self, "selected_driver", None) else [])
            ),
        }
    ...
    @staticmethod
    def get_battle_group(ordered_codes, selected_codes, radius=2):
        """Return (ordered_battle_list, battle_set) for focus mode.

        ordered_codes: driver codes sorted fastest-first (track order).
        selected_codes: list of selected driver codes to focus on.
        radius: how many cars ahead/behind each selected driver to include.
        """
        if not ordered_codes or not selected_codes:
            return [], set()
        battle_set = set()
        for sel in selected_codes:
            if sel not in ordered_codes:
                continue
            i = ordered_codes.index(sel)
            lo = max(0, i - radius)
            hi = min(len(ordered_codes), i + radius + 1)
            for c in ordered_codes[lo:hi]:
                battle_set.add(c)
        ordered_battle = [c for c in ordered_codes if c in battle_set]
        return ordered_battle, battle_set

    @staticmethod
    def gap_between(progress_a, progress_b):
        """Gap from progress_m values. Matches DriverInfo/Leaderboard math."""
        try:
            dist_m = abs(float(progress_a) - float(progress_b))  # progress is in metres
            time_s = dist_m / 55.56  # ~200 km/h reference speed (55.56 m/s)
            return dist_m, time_s
        except Exception:
            return 0.0, 0.0

    # Battle connectors only make sense for genuine proximity. Beyond this many
    # seconds (or a large screen distance) a tether becomes a distracting streak
    # across the map, e.g. when a lapped car is adjacent in track order.
    MAX_CONNECTOR_SECONDS = 3.0
    MAX_CONNECTOR_SCREEN_FRAC = 0.45

    @classmethod
    def should_draw_connector(cls, time_s, screen_dist, screen_w=1920, screen_h=1080,
                              max_time_s=None, max_screen_frac=None):
        """Return True only when a battle connector is worth drawing.

        Guards against long-distance streaks: two cars can be adjacent in track
        order yet physically far apart (lapped traffic, pit-stop gaps).
        """
        if max_time_s is None:
            max_time_s = cls.MAX_CONNECTOR_SECONDS
        if max_screen_frac is None:
            max_screen_frac = cls.MAX_CONNECTOR_SCREEN_FRAC
        try:
            t = float(time_s)
            if not math.isfinite(t) or t > float(max_time_s):
                return False
            limit = float(max_screen_frac) * min(screen_w, screen_h)
            if screen_dist is not None and float(screen_dist) > limit:
                return False
            return True
        except Exception:
            return False
    ...
    @staticmethod
    def _rects_overlap(rect, rects):
        l, b, r, t = rect
        for l2, b2, r2, t2 in rects:
            if l < r2 and r > l2 and b < t2 and t > b2:
                return True
        return False

    @staticmethod
    def _label_rect(lx, ly, w, h, anchor_x):
        if anchor_x == "left":
            return (lx, ly - h / 2.0, lx + w, ly + h / 2.0)
        return (lx - w, ly - h / 2.0, lx, ly + h / 2.0)

    @staticmethod
    def _place_label(sx, sy, snx, sny, text, occupied, forbidden, base=34.0):
        """Find a collision-free spot for a driver label, or None.

        Tries progressively larger outward offsets, then falls back to the
        inward side, so labels spread out instead of piling up.
        """
        w = len(text) * 7.0 + 10.0
        h = 15.0
        for step in range(4):
            ...
            for sign in (1.0, -1.0):
                lx = sx + snx * off * sign
                ly = sy + sny * off * sign
                anchor_x = "left" if snx * sign >= 0 else "right"
                rect = F1RaceReplayWindow._label_rect(lx, ly, w, h, anchor_x)
                if F1RaceReplayWindow._rects_overlap(rect, forbidden):
                    continue
                if F1RaceReplayWindow._rects_overlap(rect, occupied):
                    continue
                occupied.append(rect)
                return lx, ly, anchor_x
        return None

    def _forbidden_label_zones(self):
        """Screen regions owned by UI panels; labels must not enter them."""
        return [
            (0.0, 0.0, float(self.left_ui_margin), float(self.height)),
            (self.width - self.right_ui_margin, 0.0, float(self.width), float(self.height)),
            (0.0, 0.0, float(self.width), self.bottom_ui_reserved - 8.0),
            (0.0, self.height - self.top_ui_reserved + 8.0, float(self.width), float(self.height)),
        ]

    def _compute_driver_progress(self, frame):
        """Project every driver to along-track progress (metres since start)."""
        progress = {}
    ...
    def _get_energy_series(self, code):
        """Lazily build and cache a compact estimated-energy series for a driver.

        Arrays are aligned 1:1 with ``self.frames`` so the current frame index
        can index straight into them.
        """
        cached = self._energy_cache.get(code)
        ...
        for f in self.frames:
            pos = f["drivers"].get(code)
            if pos is not None:
                last = pos
            elif last is not None:
                pos = last
            t.append(f.get("t", 0.0))
            if pos is None:
                throttle.append(0.0); brake.append(0.0); speed.append(0.0); lap.append(1)
            else:
                throttle.append(pos.get("throttle", 0.0))
                brake.append(pos.get("brake", 0.0))
                speed.append(pos.get("speed", 0.0))
                lap.append(pos.get("lap", 1))

        series = estimate_arrays(t, throttle, brake, speed, lap, config=self._model_config)
        self._energy_cache[code] = series
        return series

    def _draw_minimap(self, frame, selected_drivers, battle_set, battle_ordered):
        """Small broadcast circuit map with the whole field and the focus car."""
        if not getattr(self, "visible_hud", True) or not self.screen_inner_points:
            return
        w, h = 216.0, 176.0
        cx = self.width - self.right_ui_margin / 2 - 2
        cy = 60 + h / 2
        left, right = cx - w / 2, cx + w / 2
        bottom, top = cy - h / 2, cy + h / 2

        arcade.draw_rect_filled(arcade.XYWH(cx, cy, w, h), (*PANEL, 220))
        arcade.draw_rect_outline(arcade.XYWH(cx, cy, w, h), EDGE, 1)
        arcade.draw_rect_filled(arcade.XYWH(cx, top - 3, w, 4), F1_RED)
        arcade.Text("MAP", left + 8, top - 10, MUTED, 9, bold=True, anchor_y="top").draw()

        pts = self.screen_inner_points
        xs = [p[0] for p in pts]
        ys = [p[1] for p in pts]
        ...
        inner_left, inner_right = left + 8, right - 8
        inner_bottom, inner_top = bottom + 8, top - 22
        scale = min((inner_right - inner_left) / bw, (inner_top - inner_bottom) / bh)
        ox = inner_left + ((inner_right - inner_left) - bw * scale) / 2 - minx * scale
        oy = inner_bottom + ((inner_top - inner_bottom) - bh * scale) / 2 - miny * scale

        outline = [(px * scale + ox, py * scale + oy) for px, py in pts]
        arcade.draw_line_strip(outline, (92, 92, 104), 1.4)

        # connection line through the battle group
        if battle_ordered and len(battle_ordered) > 1:
            line = []
            for code in battle_ordered:
                pos = frame["drivers"].get(code)
                if not pos:
                    continue
                sx, sy = self.world_to_screen(pos["x"], pos["y"])
                line.append((sx * scale + ox, sy * scale + oy))
            if len(line) > 1:
                arcade.draw_line_strip(line, (*ELECTRIC, 120), 1)

        for code, pos in frame["drivers"].items():
            sx, sy = self.world_to_screen(pos["x"], pos["y"])
            mx, my = sx * scale + ox, sy * scale + oy
            colour = self.driver_colors.get(code, (200, 200, 200))
            if code in selected_drivers:
                arcade.draw_circle_filled(mx, my, 4.5, colour)
                arcade.draw_circle_outline(mx, my, 6.0, (255, 255, 255), 1.4)
            elif code in battle_set:
    ...
    def _build_focus_report(self, frame, selected_drivers, ordered_codes, driver_progress):
        ...
        idx = ordered_codes.index(focus_code)
        ahead = ordered_codes[idx - 1] if idx > 0 else None
        behind = ordered_codes[idx + 1] if idx < len(ordered_codes) - 1 else None

        gap_ahead_s = None
        if ahead is not None:
            _, gap_ahead_s = self.gap_between(
                driver_progress.get(ahead, 0.0), driver_progress.get(focus_code, 0.0)
            )
        self._focus_gap_ahead_s = gap_ahead_s

        frame_idx = min(int(self.frame_index), self.n_frames - 1)
        energy = self._get_energy_series(focus_code).snapshot(frame_idx)

        fdrv = frame["drivers"].get(focus_code, {})
        adrv = frame["drivers"].get(ahead, {}) if ahead else {}
        speed_delta = float(fdrv.get("speed", 0.0)) - float(adrv.get("speed", 0.0))
        drs = fdrv.get("drs", 0) in (8, 10, 12, 14)
        tyre_life = float(fdrv.get("tyre_life", 0.0) or 0.0)
        lap = int(fdrv.get("lap", energy.lap) or energy.lap)
        laps_remaining = (self.total_laps - lap) if self.total_laps else None
        position = idx + 1  # authoritative live track order (matches leaderboard/focus banner)

        report = self.race_engineer.build_report(
            code=focus_code,
            energy=energy,
            gap_ahead_s=gap_ahead_s,
            gap_behind_s=(
                self.gap_between(driver_progress.get(focus_code, 0.0),
                                 driver_progress.get(behind, 0.0))[1]
        ...
        report.position = int(position)  # consumed by the HUD header

        # Feed the rival's currently public channels into the 40-state model.
        # DRS is used as the active-aero proxy when no aero channel is present.
        if ahead is not None:
            rival = frame["drivers"].get(ahead, {})
            rival_obs = RivalTelemetry(
    ...
    def update_scaling(self, screen_w, screen_h):
        ...
        # Reserve left/right UI margins AND the top/bottom HUD bands before
        # applying padding so the track never renders underneath UI panels.
        inner_w = max(1.0, screen_w - self.left_ui_margin - self.right_ui_margin)
        avail_h = max(80.0, screen_h - self.top_ui_reserved - self.bottom_ui_reserved)
        usable_w = inner_w * (1 - 2 * padding)
        usable_h = avail_h * (1 - 2 * padding)
        ...
        # Center within the usable area between the reserved UI bands.
        screen_cx = self.left_ui_margin + inner_w / 2
        screen_cy = self.bottom_ui_reserved + avail_h / 2
    ...
    def on_draw(self):
        ...
        # Compute track-order progress once and reuse for battle + leaderboard.
        # This avoids 2x KD-tree lookups per frame (was done separately below).
        driver_progress = self._compute_driver_progress(frame)
        ordered_codes = sorted(driver_progress, key=lambda c: driver_progress[c], reverse=True)

        # Battle group: selected + N ahead/behind in track order
        if self.focus_mode and selected_drivers:
            battle_ordered, battle_set = self.get_battle_group(
                ordered_codes, selected_drivers, radius=self.focus_radius
            )
        else:
            battle_ordered, battle_set = [], set()
        self._battle_ordered = battle_ordered
        self._battle_codes_set = battle_set
        focus_active = len(battle_set) > 0
        self._battle_screen_pos = {}

        forbidden = self._forbidden_label_zones()
        label_candidates = []
        for i, (code, pos) in enumerate(frame["drivers"].items()):
            sx, sy = self.world_to_screen(pos["x"], pos["y"])
            self._battle_screen_pos[code] = (sx, sy)
            color = self.driver_colors.get(code, arcade.color.WHITE)

            is_selected = code in selected_drivers
            is_battle = code in battle_set

            # Label logic: in focus mode always label battle cars so you can
            # track who is attacking / defending your driver.
            want_label = self.show_driver_labels or is_selected or (focus_active and is_battle)

            if want_label:
                # Find closest point index on reference track (Optimized KD-Tree)
                _, q_idx = self.track_tree.query([pos["x"], pos["y"]])
                q_idx = int(q_idx)
                nx = self._ref_nx[q_idx]
                ny = self._ref_ny[q_idx]
                # Orient the label away from the circuit centre.
                dcx = (self.x_min + self.x_max) / 2.0
                dcy = (self.y_min + self.y_max) / 2.0
                if nx * (pos["x"] - dcx) + ny * (pos["y"] - dcy) < 0:
                    nx, ny = -nx, -ny
                ...
                if self._rot_rad:
                    ...
                    snx, sny = nx, ny
                label_candidates.append((code, sx, sy, snx, sny, color))

            if not focus_active:
                arcade.draw_circle_filled(sx, sy, 6, color)
            else:
                if is_selected:
                    # Hero car: glow halo + white ring + larger body (F1 TV style)
                    arcade.draw_circle_filled(sx, sy, 13, (*color[:3], 70))
                    arcade.draw_circle_filled(sx, sy, 8, color)
                    arcade.draw_circle_outline(sx, sy, 9, arcade.color.WHITE, 2)
                elif is_battle:
                    # Rival in proximity: full colour, slightly larger, team ring
                    arcade.draw_circle_filled(sx, sy, 7, color)
                    arcade.draw_circle_outline(sx, sy, 8, color, 1)
                else:
                    # Out of focus: dim to push attention to the battle
                    arcade.draw_circle_filled(sx, sy, 4, (75, 75, 75))

        # 3a. Battle connectors: draw tether lines first, then place decluttered
        # labels and gap pills that avoid each other and the UI panels.
        occupied = []
        pill_candidates = []
        if focus_active and len(battle_ordered) > 1:
            for a, b in zip(battle_ordered, battle_ordered[1:]):
                if a not in self._battle_screen_pos or b not in self._battle_screen_pos:
                    continue
                ax, ay = self._battle_screen_pos[a]
                bx, by = self._battle_screen_pos[b]
                dist_m, time_s = self.gap_between(
                    driver_progress.get(a, 0.0), driver_progress.get(b, 0.0)
                )
                # Skip tethers that span too much time or screen distance so we
                # don't streak lines across the map between distant cars.
                screen_dist = ((ax - bx) ** 2 + (ay - by) ** 2) ** 0.5
                if not self.should_draw_connector(
                    time_s, screen_dist, screen_w=self.width, screen_h=self.height
                ):
                    continue
                # Threat colour: red = wheel-to-wheel, orange = DRS range
                if time_s < 0.6:
                    line_col = (255, 60, 60)
                elif time_s < 1.2:
                    line_col = (255, 165, 0)
                else:
                    line_col = (120, 120, 120)
                arcade.draw_line(ax, ay, bx, by, line_col, 2)
                seg = max(1.0, screen_dist)
                pill_candidates.append(
                    ((ax + bx) / 2, (ay + by) / 2, -(by - ay) / seg, (bx - ax) / seg,
                     f"+{time_s:.1f}s", line_col)
                )

        # Labels win collisions; each gets a dark chip so text stays readable.
        for (code, sx, sy, snx, sny, color) in label_candidates:
            placed = self._place_label(sx, sy, snx, sny, code, occupied, forbidden)
            if placed is None:
                continue
            lx, ly, anchor_x = placed
            w = len(code) * 7.0 + 10.0
            rect = self._label_rect(lx, ly, w, 15.0, anchor_x)
            arcade.draw_rect_filled(
                arcade.XYWH((rect[0] + rect[2]) / 2, (rect[1] + rect[3]) / 2,
                            rect[2] - rect[0], rect[3] - rect[1]),
                (10, 10, 14, 195),
            )
            arcade.draw_line(sx, sy, lx, ly, color, 1)
            tx = lx + 4 if anchor_x == "left" else lx - 4
            arcade.draw_text(code, tx, ly, color, 10, anchor_x=anchor_x,
                             anchor_y="center", bold=True)

        # Gap pills avoid labels and one another; if crowded, the line stands in.
        for (mx, my, px, py, text, line_col) in pill_candidates:
            pw, ph = 46.0, 16.0
            for off in (0.0, 18.0, -18.0, 34.0, -34.0):
                cx = mx + px * off
                cy = my + py * off
                rect = (cx - pw / 2, cy - ph / 2, cx + pw / 2, cy + ph / 2)
                if self._rects_overlap(rect, forbidden) or self._rects_overlap(rect, occupied):
                    continue
                occupied.append(rect)
                arcade.draw_rect_filled(arcade.XYWH(cx, cy, pw, ph), (0, 0, 0, 205))
                arcade.draw_rect_outline(arcade.XYWH(cx, cy, pw, ph), line_col, 1)
                arcade.draw_text(text, cx, cy, arcade.color.WHITE, 9,
        ...
        # driver_progress already computed before car drawing for focus mode.
        # Reuse it here (avoids 2nd full KD-tree pass per frame).
        ...
        # Focus banner (top-center): who you're tracking + immediate threat
        if getattr(self, "_battle_ordered", None) and getattr(self, "visible_hud", True):
            try:
                ordered = self._battle_ordered
                # Show banner for first selected driver in battle order
                focus_code = None
                for c in selected_drivers:
                    if c in ordered:
                        ...
                        break
                if focus_code:
                    fi = ordered.index(focus_code)
                    pos_overall = ordered_codes.index(focus_code) + 1 if focus_code in ordered_codes else -1
                    ahead_txt, behind_txt = "clear", "clear"
                    if fi > 0:
                        a = ordered[fi - 1]
                        _, ts = self.gap_between(
                            driver_progress.get(a, 0), driver_progress.get(focus_code, 0)
                        )
                        ahead_txt = f"{a} +{ts:.1f}s ahead"
                    if fi < len(ordered) - 1:
                        b = ordered[fi + 1]
                        _, ts = self.gap_between(
                            driver_progress.get(focus_code, 0), driver_progress.get(b, 0)
                        )
                        behind_txt = f"{b} -{ts:.1f}s behind"
                    team_col = self.driver_colors.get(focus_code, arcade.color.WHITE)
                    banner = f"FOCUS {focus_code}  P{pos_overall}  •  {ahead_txt}  •  {behind_txt}  •  [F]"
                    bw, bh = min(560, self.width - 40), 26
                    # Drop below the session-info banner when it is visible so the
                    # two top-centre banners do not overlap (it is drawn after us).
                    session_visible = bool(
                        getattr(self.session_info_comp, "visible", False)
                        and getattr(self.session_info_comp, "session_info", None)
                    )
                    bx = self.width / 2
                    by = self.height - (96 if session_visible else 24)
                    arcade.draw_rect_filled(arcade.XYWH(bx, by, bw, bh), (0, 0, 0, 190))
                    arcade.draw_rect_outline(arcade.XYWH(bx, by, bw, bh), team_col, 2)
                    # Team colour tick on left edge
                    arcade.draw_rect_filled(arcade.XYWH(bx - bw / 2 + 4, by, 6, bh), team_col)
                    arcade.draw_text(banner, bx, by, arcade.color.WHITE, 12,
                                     anchor_x="center", anchor_y="center", bold=True)
            except Exception:
                pass
        ...
        except Exception as exc:  # never let the intelligence layer blank the frame
            if not getattr(self, "_intel_error_logged", False):
                print(f"[race-engineer] render error: {exc}")
                self._intel_error_logged = True
