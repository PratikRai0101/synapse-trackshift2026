...
def enable_cache():
    ...
    # FastF1 expires its HTTP cache after 12 hours, which forces slow
    # re-downloads (its rate limiter can make one schedule fetch take ~75s).
    # Keep raw responses valid far longer so cached data is reused offline.
    try:
        sess = getattr(fastf1.Cache, "_requests_session_cached", None)
        if sess is not None and getattr(sess, "settings", None) is not None:
            sess.settings.expire_after = timedelta(days=365)
    except Exception:
        pass
...
def load_session(year, round_number, session_type="R"):
    ...
    # of MB (taking ~75s) even when the session is already on disk. Try an
...
def get_race_weekends_by_year(year):
    """Returns a list of race weekends for a given year (cached locally)."""
    cache_file = os.path.join("computed_data", f"schedule_{int(year)}.json")
...
def get_all_unique_race_names(start_year=2018, end_year=None): #update as necessary
    "Return a list of all unique race locations (cached locally)"
    if end_year is None:
        end_year = date.today().year
    cache_file = os.path.join("computed_data", "race_names.json")
    if os.path.exists(cache_file):
        try:
            with open(cache_file) as fh:
                cached = json.load(fh)
    ...
    for year in range(start_year, end_year+1):
        try:
            for ev in get_race_weekends_by_year(year):
                name = str(ev.get("event_name", "")).strip()
                if name:
                    race_names.add(name)
        except Exception:
            continue

    result = sorted(race_names)
    try:
        os.makedirs("computed_data", exist_ok=True)
        with open(cache_file, "w") as fh:
            json.dump(result, fh)
