...
class WeatherComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        if not self.info and not getattr(window, "has_weather", False):
            return
        # Broadcast panel behind the weather readout
        wx = self.left + self.width / 2
        wy = panel_top - 71
        arcade.draw_rect_filled(arcade.XYWH(wx, wy, self.width, 158), (*PANEL, 205))
        arcade.draw_rect_outline(arcade.XYWH(wx, wy, self.width, 158), EDGE, 1)
        arcade.draw_rect_filled(arcade.XYWH(self.left + 4, wy, 4, 158), ELECTRIC)
        arcade.Text("WEATHER", self.left + 12, panel_top - 10, TEXT, 16, bold=True, anchor_y="top").draw()
        ...
        self._text.font_size = 16; self._text.bold = True; self._text.color = TEXT
        ...
        for idx, (label, value, icon_key) in enumerate(weather_lines):
            ...
            self._text.font_size = 14; self._text.bold = False; self._text.color = MUTED
...
class LeaderboardComponent(BaseComponent):
    ...
    def _calculate_gaps(self):
        ...
        for idx, (code, _, pos, progress_m) in enumerate(self.entries):
            ...
            try:
                ...
                dist_to_leader = raw_to_leader  # metres
    ...
    def draw(self, window):
        ...
        arcade.Text("Leaderboard", self.x, leaderboard_y, TEXT, 20, bold=True, anchor_x="left", anchor_y="top").draw()
        arcade.draw_rect_filled(
            arcade.XYWH(self.x + self.width / 2, leaderboard_y - 26, self.width, 3), F1_RED
        )
        ...
        # Broadcast panel behind the standings
        _panel_top = leaderboard_y + 12
        _panel_bottom = leaderboard_y - 30 - len(new_entries) * self.row_height - 6
        _panel_cy = (_panel_top + _panel_bottom) / 2
        _panel_h = max(1.0, _panel_top - _panel_bottom)
        arcade.draw_rect_filled(
            arcade.XYWH(self.x + self.width / 2, _panel_cy, self.width, _panel_h), (*PANEL, 205)
        )
        arcade.draw_rect_outline(
            arcade.XYWH(self.x + self.width / 2, _panel_cy, self.width, _panel_h), EDGE, 1
        )
        ...
        for i, (code, color, pos, progress_m) in enumerate(new_entries):
            ...
            if code in self.selected:
                ...
                # Focus mode: subtly highlight battle rivals so you can see
                # who is directly attacking / defending your selected driver.
                battle_set = getattr(window, "_battle_codes_set", set())
                if battle_set and code in battle_set:
                    rect = arcade.XYWH((left_x + right_x)/2, (top_y + bottom_y)/2, right_x - left_x, top_y - bottom_y)
                    arcade.draw_rect_filled(rect, (30, 30, 38, 255))
                    # Electric accent tick on left edge for battle rivals
                    arcade.draw_rect_filled(
                        arcade.XYWH(left_x + 3, (top_y + bottom_y) / 2, 5, top_y - bottom_y - 4),
                        ELECTRIC,
                    )
                text_color = color
...
class DriverInfoComponent(BaseComponent):
    ...
    def _draw_info_box(self, window, code, driver_pos, center_y, box_width, box_height):
        ...
        arcade.draw_rect_filled(rect, (*PANEL, 228))

        team_color = window.driver_colors.get(code, arcade.color.GRAY)
        arcade.draw_rect_outline(rect, EDGE, 1)
        # Team-colour accent down the left edge (broadcast card)
        arcade.draw_rect_filled(arcade.XYWH(left + 3, center_y, 5, box_height - 4), team_color)
        ...
        if lb and hasattr(lb, "entries") and lb.entries:
            try:
                ...
                def get_gap_str(neighbor_idx, prefix, sign):
                    ...
                    dist = abs(curr_pos - n_pos)  # metres
...
class ControlsPopupComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        arcade.draw_rect_filled(rect, (*PANEL, 244))
        arcade.draw_rect_outline(rect, EDGE, 1)

        header_height = max(28, int(self.header_font_size * 2))
        header_cy = cy + self.height / 2 - header_height / 2
        arcade.draw_rect_filled(arcade.XYWH(cx, header_cy, self.width, header_height), F1_RED)
...
class SessionInfoComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        # Broadcast panel: dark glass, subtle edge, F1-red top accent.
        rect = arcade.XYWH(center_x, top_y - banner_height / 2, banner_width, banner_height)
        arcade.draw_rect_filled(rect, (*PANEL, 236))
        arcade.draw_rect_outline(rect, EDGE, 1)
        arcade.draw_rect_filled(arcade.XYWH(center_x, top_y - 3, banner_width, 6), F1_RED)
...
class RaceControlsComponent(BaseComponent):
    ...
    def _draw_button_base(self, x: float, y: float, primary: bool = False) -> None:
        """Broadcast-style circular base behind a control icon."""
        radius = self.button_size / 2 + 6
        arcade.draw_circle_filled(x, y, radius, (*PANEL, 235))
        arcade.draw_circle_outline(x, y, radius, F1_RED if primary else EDGE, 2)

    def _draw_play_icon(self, x: float, y: float):
        self._draw_button_base(x, y, primary=True)
    ...
    def _draw_speed_comp(self, x: float, y: float, speed: float):
        ...
        if 'speed+' and 'speed-' in self._control_textures:
            ...
            arcade.draw_rect_filled(rect_container, (*PANEL, 235))
            arcade.draw_rect_outline(rect_container, EDGE, 1)
